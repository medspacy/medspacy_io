...
def test_data_paths():
    ...
    return {
        'xml_file': project_root / "tests/data/ehost_split_anno/saved/doc2.txt.knowtator.xml",
        'txt_file': project_root / "tests/data/ehost_split_anno/corpus/doc2.txt",
        'schema_file': project_root / "tests/data/ehost_split_anno/config/projectschema.xml"
    }
...
def test_split_annotation(nlp, test_data_paths):
    ...
    assert len(split_span) == 3, "Expected span to include text when store_anno_string=True"
    span_text = split_span[2]
    assert span_text == "skin was closed at the level of the skin ... incised", \
    ...
    assert classes['EHOST_Instance_17'][0] == 'CONCEPT', \
...
def test_split_annotation_text_verification(nlp, test_data_paths):
    ...
    reader = EhostDocReader(nlp=nlp, schema_file=str(schema_file), store_anno_string=True)
    spans, classes, attributes, relations = reader.parse_to_dicts(str(xml_file), sort_spans=False)
    ...
    assert span1_text == "skin was closed at the level of the skin", \
