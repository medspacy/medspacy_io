...
class TestEhostReader(unittest.TestCase):
    ...
    def test_to_sents_df(self):
        ereader = EhostDocReader(nlp=self.nlp, schema_file='data/ehost_test_corpus2/config/projectschema.xml', support_overlap=True)
    ...
    def test_to_sents_df_on_attr_value(self):
        ereader = EhostDocReader(nlp=self.nlp, schema_file='data/ehost_test_corpus2/config/projectschema.xml', support_overlap=True)
        doc = ereader.read('data/ehost_test_corpus2/corpus/doc1.txt')
        df = Vectorizer.to_sents_df(doc, type_filter={"Nonspecific_SSTI": {'status': {'present': 'PRES_Nonspecific_SSTI'}}})
        print(df.shape)
        print(df)
        assert (df.shape[0] == 4)
        assert (df.iloc[0].y == 'PRES_Nonspecific_SSTI')
        df = Vectorizer.to_sents_df(doc, sent_window=2,type_filter={"Nonspecific_SSTI": {'status': {'present': 'PRES_Nonspecific_SSTI'}}})
        print(df)
        assert (df.shape[0] == 3)
        df = Vectorizer.to_sents_df(doc, sent_window=2,type_filter={"Nonspecific_SSTI": {'status': {'present': 'PRES_Nonspecific_SSTI'}}},track_doc_name=True)
        print(df)
        assert (df.shape[0] == 3)
        assert (df.shape[1] == 4)
    ...
    def test_to_sents_df_on_attr_value2(self):
        ereader = EhostDocReader(nlp=self.nlp, schema_file='data/ehost_test_corpus2/config/projectschema.xml', support_overlap=True)
        doc = ereader.read('data/ehost_test_corpus2/corpus/doc1.txt')
        df = Vectorizer.to_sents_df(doc, type_filter={"Nonspecific_SSTI": {'status': {'negated': 'PRES_Nonspecific_SSTI'}}})
        print(df.shape)
        print(df)
        assert (df.shape[0] == 4)
        assert (df.iloc[0].y == 'NEG')
        df = Vectorizer.to_sents_df(doc, sent_window=2, type_filter={"Nonspecific_SSTI": {'status': {'negated': 'PRES_Nonspecific_SSTI'}}})
        print(df.shape)
        assert (df.shape[0] == 3)
        print(df)
        assert (df.iloc[0].y == 'NEG')
    ...
    def test_to_sents_df_on_attr_value3(self):
        ereader = EhostDocReader(nlp=self.nlp, schema_file='data/ehost_test_corpus2/config/projectschema.xml', support_overlap=True)
        doc = ereader.read('data/ehost_test_corpus2/corpus/doc1.txt')
        df = Vectorizer.to_sents_df(doc, type_filter={"Nonspecific_SSTI": {'status': {'present': 'PRES_Nonspecific_SSTI'},'test': {'v2': "TYPE_1"}}})
        print(df.shape)
        print(df)
        assert (df.shape[0] == 5)
        assert (df.iloc[0].y == 'PRES_Nonspecific_SSTI')
        assert (df.iloc[1].y == 'TYPE_1')
        df = Vectorizer.to_sents_df(doc, sent_window=2, type_filter={"Nonspecific_SSTI": {'status': {'present': 'PRES_Nonspecific_SSTI'}, 'test': {'v2': "TYPE_1"}}})
        print(df.shape)
        assert (df.shape[0] == 5)
    ...
    def test_docs_to_sents_df(self):
        ...
        dir_reader = EhostDirReader(nlp=self.nlp, support_overlap=False, recursive=True, schema_file='data/ehost_test_corpus/config/projectschema.xml')
    ...
    def test_docs_to_sents_df2(self):
        dir_reader = EhostDirReader(nlp=self.nlp, support_overlap=True, recursive=True, schema_file='data/ehost_test_corpus2/config/projectschema.xml')
    ...
    def test_get_output_labels(self):
        type_filter = {"Nonspecific_SSTI": {'status': {'present': 'PRES_Nonspecific_SSTI', 'historical': 'HIS_Nonspecific_SSTI'},'SSI': 'SSI'}}
    ...
    def test_to_seq_data_dict_on_types(self):
        ...
        assert (res['PreAnnotated'][1][9]=='O')#'PreAnnotated')
        assert (res['PreAnnotated'][1][10]=='O')#'PreAnnotated')
    ...
    def test_to_seq_data_dict_on_types2(self):
        ...
        assert(res['PreAnnotated'][0][17]=='O')#'PreAnnotated')
        assert(res['PreAnnotated'][0][18]=='O')#'PreAnnotated')
    ...
    def test_to_seq_data_dict_on_types3(self):
        ...
        assert(res['PreAnnotated'][0][16]=='NULL')#'PreAnnotated')
        assert(res['PreAnnotated'][0][17]=='NULL')#'PreAnnotated')
    ...
    def test_to_seq_data_dict_on_types4(self):
        ...
        assert(res['PreAnnotated'][0][18]=='O')#'PreAnnotated')
        assert(res['PreAnnotated'][0][19]=='PreAnnotated')
        assert(res['Nonspecific_SSTI'][0][25]=='O')#'[SEP]')
        assert(res['Nonspecific_SSTI'][0][26]=='O')#'Nonspecific_SSTI')
        assert(res['Nonspecific_SSTI'][0][27]=='[SEP]')#'Nonspecific_SSTI')
        assert(res['Nonspecific_SSTI'][0][28]=='Nonspecific_SSTI')#'O')
        ...
        assert(res['Nonspecific_SSTI'][1][17]=='O')#'[SEP]')
        assert(res['Nonspecific_SSTI'][1][18]=='O')#'Nonspecific_SSTI')
        assert(res['Nonspecific_SSTI'][1][19]=='[SEP]')#'Nonspecific_SSTI')
        assert(res['Nonspecific_SSTI'][1][20]=='Nonspecific_SSTI')#'O')
    ...
    def test_to_seq_data_dict_on_types5(self):
        ...
        assert(res['PREANNO'][1][9]=='O')#'PREANNO')
        assert(res['PREANNO'][1][10]=='O')#'PREANNO')
        assert(res['PREANNO'][1][11]=='PREANNO')
        assert(res['PREANNO'][1][12]=='PREANNO')#'O')
