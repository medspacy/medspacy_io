...
class TestEhostReader(unittest.TestCase):
    ...
    def test_set_attributes(self): #PASSES
        if Doc.has_extension("concepts"):
            Doc.remove_extension("concepts")
    ...
    def test_read_doc_name(self): #PASSED
        ereader = EhostDocReader(nlp=English(), schema_file='data/ehost_test_corpus/config/projectschema.xml')
        doc = ereader.read('data/ehost_test_corpus/corpus/doc1.txt')
        ...
        ereader = EhostDocReader(nlp=English(), schema_file='data/ehost_test_corpus/config/projectschema.xml',doc_name_depth=2)
        doc = ereader.read('data/ehost_test_corpus/corpus/doc1.txt')
    ...
    def test_check_spans(self): #PASSED
        if Doc.has_extension("concepts"):
            Doc.remove_extension("concepts")
        ereader = EhostDocReader(nlp=English(), schema_file='data/ehost_test_corpus/config/projectschema.xml',
    ...
    def test_check_spans_overlapSpan(self):#PASSED
    ...
    def test_dir_reader(self): #PASSED
        if Doc.has_extension("concepts"):
            Doc.remove_extension("concepts")
    ...
    def test_dir_reader_overlapSpan(self):#PASSED
        if Doc.has_extension("concepts"):
            Doc.remove_extension("concepts")
        dir_reader = EhostDirReader(nlp=English(), support_overlap=True,recursive=True, schema_file='data/ehost_test_corpus/config/projectschema.xml')
        docs = dir_reader.read(txt_dir='data/ehost_test_corpus/')
        assert (len(docs) == 2)
        for doc in docs:
            assert (len(doc.spans) == 7)
