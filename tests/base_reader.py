...
class BaseDocReader(object):
    ...
    def find_start_token(self, start: int, left_token_offset: int,
        ...
        """
        # print('find {} offset between:\t{}[{}]-{}[{}]'.format(start,left_token_offset,doc[left_token_offset].idx,
        #                                                       right_token_offset, doc[right_token_offset].idx))
        if right_token_offset >= left_token_offset:
            mid = int(left_token_offset + (right_token_offset - left_token_offset) / 2)
            # print(start, doc[mid].idx, mid)
            # if start==579:
            #     print('warn')
            ...
            if doc[mid].idx <= start and start < doc[mid].idx + len(doc[mid]):
                ...
                return mid
            elif mid==left_token_offset or mid==right_token_offset :
                return mid
