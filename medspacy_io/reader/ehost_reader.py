...
class EhostDocReader(BaseDocReader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.store_anno_string = (
            store_anno_string  # Controls whether to include span text in output
        )
        self.use_adjudication = (
            use_adjudication  # Initialize use_adjudication attribute
        )
    ...
    def set_attributes(
        self, schema_file: Union[str, Path] = "", encoding: Union[str, None] = None
    ) -> Set:
        """
        ...
        if schema_file is not None:
            ...
            for attr_def in root.iter("attributeDef"):
                ...
                name = (
                    "ANNOT_" + name
                )  # This is to make annotation attribute different from spacy
                ...
                if name not in attr_names and not Span.has_extension(name):
                    ...
                    )  # initialize the attr value for each attributes
                    attr_names.add(name)
    ...
    def parse_to_dicts(
        ...
        for event, ele in iter:
            if ele.tag == "annotation":
                ...
                ele.tag == "hasSlotMention"
            ):  # all attributes and relations info are here
                if id not in classes:
                    classes[id] = ["", []]
                ...
                # </complexSlotMention>
                rel_id, label, components = self.parse_relation_tag(
                    ele, iter
                )  # !! rel_id is relation ID not reference ID !! eHost annot file XML has only destination ref
