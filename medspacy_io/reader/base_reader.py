...
class BaseDocReader(object):
    ...
    def get_doc_name(self, txt_file: Union[str, Path], doc_name_depth: int = 0) -> str:
        ...
        if doc_name_depth == -1:
            ...
            base_name = str(txt_file_path.absolute().relative_to(txt_file_path.absolute().parents[doc_name_depth]))
        return base_name
    ...
    def process_without_overlaps(
        ...
        for id, span_tuple in sorted_spans:
            ...
            if token_start >= 0 and token_end > 0:
                ...
                raise OverflowError(
                    "The span of the annotation: {}[{}:{}] is out of document boundary.".format(classes[id][0], start, end)
                )
        doc.ents = existing_entities + new_entities
        rels = []
        # print(relations)
        # print(classes)
        for rel_source, (rel_name, rel_target) in relations.items():
            source_span = None
            target_span = None
            for ent in doc.ents:
            ...
            if start < doc[0].idx:
                ...
                self.logger.debug(
                    "To find {} between token_start ({}[{}]) and  token_right_bound({}[{}])".format(
                        start, token_start, doc[token_start].idx, token_right_bound, doc[token_right_bound].idx
                    ),
                )
                token_start = self.find_start_token(start, token_start, token_right_bound, doc)
                ...
                if end >= doc[-1].idx + doc[-1].__len__():
                    self.logger.debug(
                        "end  ({}) >= doc[-1].idx ({}) + doc[-1].__len__() ({})".format(end, doc[-1].idx, doc[-1].__len__())
                    )
                    token_end = token_right_bound + 1
                else:
                    self.logger.debug(
                        "To find token_end starts from {} between token_start ({}[{}]) and  token_right_bound({}[{}])".format(
                            end, token_start, doc[token_start].idx, token_right_bound, doc[token_right_bound].idx
                        )
                    )
                    token_end = self.find_end_token(end, token_start, token_right_bound, doc)
    ...
    def find_start_token(self, start: int, token_left_bound: int, token_right_bound: int, doc: Doc) -> int:
        """
            ...
            if doc[mid].idx <= start < doc[mid].idx + len(doc[mid]):
                self.logger.debug(
                    "return mid={} when doc[{}]({}) < {} < doc[{}].idx+len(doc[{}])({})".format(
                        mid, mid, doc[mid].idx, start, mid, mid, doc[mid].idx + len(doc[mid])
                    )
                )
                return mid
...
class BaseDirReader:
    ...
    def __init__(
        self, txt_extension: str = "txt", recursive: bool = False, nlp: Language = None, docReaderClass: Type = None, **kwargs
    ):
        """
